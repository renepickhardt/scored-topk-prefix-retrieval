...
public class ScoredArrayTrieTest {
	...
	// for (int i = ScoredArrayTrieNode.MIN_CHAR; i <
	// ScoredArrayTrieNode.MAX_CHAR; i++) {
	// T.add("" + (char) i, i);
	// }
	//
	// result = T.root.calcTopChilds();
	...
	public void testGetTopK() {
		// 15 is bigger thant the cache in ScoredArrayTrie so overflows may
		// occure
		this.setupTries(5);
		this.T.root.recSetMaxScore();
		this.T.root.recSetTopChilds();

		// empty results
		this.checkResults("X");

		this.addToTries("which", 249968);
		this.addToTries("when", 73492);
		this.addToTries("where", 55024);
		this.addToTries("while", 44662);
		this.addToTries("what", 24407);
		this.addToTries("whose", 11803);
		this.addToTries("white", 8883);
		this.addToTries("whether", 8830);
		this.addToTries("whom", 8527);
		this.addToTries("whole", 6224);
		this.addToTries("whereas", 3548);
		this.addToTries("whilst", 1494);
		this.addToTries("whereby", 1050);
		this.T.root.recSetMaxScore();
		this.T.root.recSetTopChilds();

		this.checkResults("w");
		this.checkResults("wh");
		this.checkResults("whe");
		this.checkResults("wher");
		this.checkResults("where");
		this.checkResults("X");
		this.T.root.recSetMaxScore();
		this.T.root.recSetTopChilds();
		...
		this.addAlphabeth("A", 10);
		this.addAlphabeth("AA", 11);
		this.addAlphabeth("AAA", 12);
		this.T.root.recSetMaxScore();
		this.T.root.recSetTopChilds();
		...
		this.checkResults("B");
		this.checkResults("X");
		this.checkResults("AA");
		this.checkResults("AB");
		this.checkResults("AX");
		this.checkResults("AAA");
	}
	...
	public void setupTries(int k) {
		this.maxResults = k;
		this.sTree = new SuggestTree(this.maxResults);
		this.T = new ScoredArrayTrie();
	}

	public void addToTries(String s, int score) {
		this.sTree.put(s, score);
		this.T.add(s, score);
	}
	...
	public void addAlphabeth(String prefix, int offset) {
		for (int i = 1; i < 40; i++) {
			this.addToTries(prefix + (char) ('A' + i), offset + 10 * i);
		}
